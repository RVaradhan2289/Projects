...
const uri = `mongodb+srv://${MONGO_DB_USERNAME}:${MONGO_DB_PASSWORD}@cmsc335cluster.msfm8sx.mongodb.net/`;
...
async function addDriver(client, databaseAndCollection, driver, drivers) {
    const existingDriver = await client.db(databaseAndCollection.db).collection(databaseAndCollection.collection)
    .findOne({driverId:driver.driverId});
    if(!existingDriver) {
